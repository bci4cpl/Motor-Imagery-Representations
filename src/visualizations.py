...
def plot_auc_vs_cluster_separation(X_csp, X_reduced, y_label, days_labels, clf_loaded, start_test_day, end_test_day, dim=6, reducer='CSP', directory='None'):
    unique_days = np.unique(days_labels)
    aucs = []
    inter_variances = []
    intra_variances_idle = []
    ...
    # Correlations
    corr_auc_inter, _ = pearsonr(aucs, inter_variances)
    corr_auc_intra_idle, _ = pearsonr(aucs, intra_variances_idle)
    ...
    ax1.plot(unique_days, aucs, label='AUC', marker='o', color='tab:blue')
    ax1.tick_params(axis='y', labelcolor='tab:blue')
    ...
    ax1_twin.legend([f"Inter-Cluster Distance (corr: {corr_auc_inter:.2f})"], loc='upper right')
    ax1.set_title(f"AUC vs Inter-Cluster Distance in {dim}D {reducer} space Over Days {start_test_day}-{end_test_day}")

    # Second subplot: Accuracy vs Intra-Cluster Distance (Idle)
    ax2.set_xlabel('Days', labelpad=3)
    ax2.set_ylabel('AUC', color='tab:blue')
    ax2.plot(unique_days, aucs, label='AUC', marker='o', color='tab:blue')
    ax2.tick_params(axis='y', labelcolor='tab:blue')

    ax2_twin = ax2.twinx()
    ax2_twin.set_ylabel('Intra-Cluster Variance Idle [std]', color='tab:green')
    ax2_twin.plot(unique_days, intra_variances_idle, label='Intra-Cluster Variance Idle [std]', marker='x', color='tab:green')
    ax2_twin.tick_params(axis='y', labelcolor='tab:green')

    ax2_twin.legend([f"Intra-Cluster Variance Idle (corr: {corr_auc_intra_idle:.2f})"], loc='upper right')
    ax2.set_title(f'AUC vs Intra-Cluster Variance (Idle) in {dim}D {reducer} space Over Days {start_test_day}-{end_test_day}')

    # Third subplot: Accuracy vs Intra-Cluster Distance (Motor Imagery)
    ax3.set_xlabel('Days', labelpad=3)
    ax3.set_ylabel('AUC', color='tab:blue')
    ax3.plot(unique_days, aucs, label='AUC', marker='o', color='tab:blue')
    ax3.tick_params(axis='y', labelcolor='tab:blue')

    ax3_twin = ax3.twinx()
    ax3_twin.set_ylabel('Intra-Cluster Variance Motor Imagery [std]', color='tab:purple')
    ax3_twin.plot(unique_days, intra_variances_motor, label='Intra-Cluster Variance Motor Imagery [std]', marker='x', color='tab:purple')
    ax3_twin.tick_params(axis='y', labelcolor='tab:purple')

    ax3_twin.legend([f"Intra-Cluster Variance Motor Imagery (corr: {corr_auc_intra_motor:.2f})"], loc='upper right')
    ax3.set_title(f'AUC vs Intra-Cluster Variance (Motor Imagery) in {dim}D {reducer} space Over Days {start_test_day}-{end_test_day}')
